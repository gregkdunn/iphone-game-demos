...
- (id) init
{
	...
	if (self != nil) {
		...
		currentLevelIndex = [Storage getCurrentLevelIndex];
		highestLevelIndex = [[Storage getUserValueForKey:@"highestlevel" withDefault:@"1"] intValue];
...
- (void)onLevelSelect:(id)sender
{
	...
	[Storage setCurrentLevelIndex:[sender tag]];//Set to whatever button was pressed.
	[Storage setCurrentLivesLeft:[Storage getDefaultLives]];
	NSLog(@"LevelSelectScene->onLevelSelect->currentLivesLeft: %i", [Storage getCurrentLivesLeft]);
